            ...
            while (attempts < maxAttempts)
            {
                ...
                if (collections is not null && collections.Count > 0)
                {
                    ...
                    if (collection.Status == "ACTIVE")
                    {
                        ...
                        _context?.Logger.LogLine($"  -- trying to store: {collection.Id}");
                        _context?.Logger.LogLine($"  -- trying to store: {namePrefix}-{nameSuffix}/collectionId");

                        await StoreParameters(
                                name: $"{namePrefix}-{nameSuffix}/collectionId",
                                value: collection.Id!
                            );
                        _context?.Logger.LogLine($"  -- SUCCESS");
